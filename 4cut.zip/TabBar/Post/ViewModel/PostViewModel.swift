    ...
    func transform(input: Input) -> Output {
        input.uploadButtonTap
            .subscribe(with: self) { owner, contentValue in
                guard let postId = contentValue.2 else {
                    owner.post(content: contentValue.1, imageList: contentValue.0, router: .postContent(content: nil))
                    return
                }
                owner.post(content: contentValue.1, imageList: contentValue.0, router: .editPost(id: postId))
            }
            .disposed(by: disposeBag)
        return Output(photoButtonTap: input.photoButtonTap, popNavi: popNavi)
    }

    private func post(content: String, imageList: [UIImage], router: Router) {
        do {
                ...
                case .success(let value):
                    Observable.just(value)
                        .flatMap { photo -> Single<Result<PostContent, NetworkError>> in
                            NetworkManager.shared.callRequestWithToken(router: .postContent(content: Content(content: content, product_id: "4cut_booth", files: photo.files)))
                        }
